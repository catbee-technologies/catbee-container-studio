...
export class DockerService {
  ...
  private followProgress(stream: NodeJS.ReadableStream): Promise<DockerProgressEvent[]> {
    return new Promise((resolve, reject) => {
      const modem = this.client.modem as unknown as {
        followProgress: (
          output: NodeJS.ReadableStream,
          onFinished: (error: Error | null, result: unknown[] | undefined) => void
        ) => void;
      };

      modem.followProgress(stream, (error, result) => {
        if (error) {
          reject(error);
          return;
        }
  ...
  async pruneImages(filters?: { [key: string]: string[] }): Promise<Docker.PruneImagesInfo> {
    return this.client.pruneImages(filters ? { filters } : {});
  }

  async pruneVolumes(filters?: { [key: string]: string[] }): Promise<Docker.PruneVolumesInfo> {
  ...
  async waitContainer(containerId: string): Promise<unknown> {
    const id = this.normalizeId(containerId, 'Container id');
    const container = this.client.getContainer(id);
  ...
  async historyImage(imageId: string): Promise<unknown[]> {
    const id = this.normalizeId(imageId, 'Image id');
