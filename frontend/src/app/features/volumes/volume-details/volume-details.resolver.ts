...
import { DockerContainerInfo, DockerContainerInspectInfo, DockerVolumeInfo } from '@shared/types/docker-api.types';
...
export const volumeDetailsResolver: ResolveFn<VolumeDetailsPrefetch> = async route => {
  ...
  try {
    ...
    for (let idx = 0; idx < inspectResults.length; idx += 1) {
      ...
      const payload = result.value as DockerContainerInspectInfo;
