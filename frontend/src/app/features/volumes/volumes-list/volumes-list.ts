...
type VolumeSortKey = 'used' | 'name' | 'driver' | 'scope' | 'size' | 'created';
type SortDirection = 'asc' | 'desc';
type VolumeUsageFilter = 'all' | 'used' | 'unused';
...
export class VolumesPage {
  ...
  readonly usageFilter = signal<VolumeUsageFilter>(this.readUsageFilterPreference());
  readonly sortKey = signal<VolumeSortKey>(this.readSortKeyPreference());
  ...
  private isVolumeSortKey(value: string | null): value is VolumeSortKey {
    return (
      value === 'used' ||
      value === 'name' ||
      value === 'driver' ||
  ...
  private readSortKeyPreference(): VolumeSortKey {
    const value = this.localStorage.get(UI_STORAGE_KEYS.VOLUMES_SORT_KEY);
    return this.isVolumeSortKey(value) ? value : 'name';
  }

  private readSortDirectionPreference(): SortDirection {
    const value = this.localStorage.get(UI_STORAGE_KEYS.VOLUMES_SORT_DIRECTION);
    return this.isSortDirection(value) ? value : 'asc';
  }

  private readUsageFilterPreference(): VolumeUsageFilter {
    const value = this.localStorage.get(UI_STORAGE_KEYS.VOLUMES_USAGE_FILTER);
    return this.isVolumeUsageFilter(value) ? value : 'all';
  }
