...
type ImageSortKey = 'used' | 'repository' | 'tag' | 'id' | 'size' | 'created';
type SortDirection = 'asc' | 'desc';
type ImageUsageFilter = 'all' | 'used' | 'unused' | 'dangling';
...
export class ImagesPage {
  ...
  readonly usageFilter = signal<ImageUsageFilter>(this.readUsageFilterPreference());
  readonly sortKey = signal<ImageSortKey>(this.readSortKeyPreference());
  ...
  private isImageUsageFilter(value: string | null): value is ImageUsageFilter {
    return value === 'all' || value === 'used' || value === 'unused' || value === 'dangling';
  }

  private isImageSortKey(value: string | null): value is ImageSortKey {
    return value === 'used' || value === 'repository' || value === 'tag' || value === 'size' || value === 'created';
  }
  ...
  private readSortKeyPreference(): ImageSortKey {
    const value = this.localStorage.get(UI_STORAGE_KEYS.IMAGES_SORT_KEY);
    return this.isImageSortKey(value) ? value : 'created';
  }

  private readSortDirectionPreference(): SortDirection {
    const value = this.localStorage.get(UI_STORAGE_KEYS.IMAGES_SORT_DIRECTION);
    return this.isSortDirection(value) ? value : 'desc';
  }

  private readUsageFilterPreference(): ImageUsageFilter {
    const value = this.localStorage.get(UI_STORAGE_KEYS.IMAGES_USAGE_FILTER);
    return this.isImageUsageFilter(value) ? value : 'all';
  }
}
