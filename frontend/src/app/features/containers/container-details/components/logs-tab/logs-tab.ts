...
export class LogsTabComponent implements AfterViewInit {
  ...
  private static readonly LOG_TAIL_LINE_OPTIONS = [200, 500, 1000, 2000, 5000] as const;
  ...
  readonly logTailLines = signal(this.readLogTailPreference());
  readonly logTailLineOptions = LogsTabComponent.LOG_TAIL_LINE_OPTIONS;
  readonly showLogTimestamps = signal(this.readTimestampPreference());
  readonly wrapLogLines = signal(this.readWrapLinesPreference());
  ...
  toggleLogTimestamps(): void {
    this.showLogTimestamps.update(value => {
      const next = !value;
      // this.writeTimestampPreference(next);
  ...
  toggleWrapLines(): void {
    this.wrapLogLines.update(value => {
      const next = !value;
      // this.writeWrapLinesPreference(next);
  ...
  async onTailLinesChange(event: Event): Promise<void> {
    ...
    if (
      !LogsTabComponent.LOG_TAIL_LINE_OPTIONS.includes(
        nextValue as (typeof LogsTabComponent.LOG_TAIL_LINE_OPTIONS)[number]
      )
    ) {
      if (target) {
  ...
  private readTimestampPreference(): boolean {
    const value = this.localStorage.getBoolean(LOGS_STORAGE_KEYS.SHOW_TIMESTAMPS);
    if (value === false) {
      return false;
    }
    if (value === true) {
  ...
  private readLogTailPreference(): number {
    const raw = this.localStorage.getEnum(
      LOGS_STORAGE_KEYS.TAIL_LINES,
      LogsTabComponent.LOG_TAIL_LINE_OPTIONS.map(String)
    );
    if (!raw) {
      return LogsTabComponent.DEFAULT_LOG_TAIL_LINES;
    }

    const parsed = Number.parseInt(raw, 10);
    if (
      LogsTabComponent.LOG_TAIL_LINE_OPTIONS.includes(parsed as (typeof LogsTabComponent.LOG_TAIL_LINE_OPTIONS)[number])
    ) {
      return parsed;
    }
    return LogsTabComponent.DEFAULT_LOG_TAIL_LINES;
  ...
  private readWrapLinesPreference(): boolean {
    const value = this.localStorage.getBoolean(LOGS_STORAGE_KEYS.WRAP_LINES);
    if (value === false) {
      return false;
    }
    if (value === true) {
